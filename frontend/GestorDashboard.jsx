...
const GestorDashboard = ({ user, onLogout }) => {
  ...
  // Dados de exemplo para o dashboard do gestor (versão aprovada)
  const gestorData = {
    kpis: [
      { name: 'Visitas Realizadas', value: 25, unit: '', target: 30, status: 'success' },
      { name: 'Pendências', value: 3, unit: '', target: 0, status: 'warning' },
      { name: 'Alertas', value: 5, unit: '', target: 0, status: 'danger' },
      { name: 'Score Médio', value: 84, unit: '', target: 90, status: 'success' }
    ],
    alertasCriticos: [
      { message: 'Ruptura crítica detectada em 3 lojas', time: '2h atrás' },
      { message: 'Queda de 15% na execução - Loja Central', time: '4h atrás' }
    ],
    recentVisits: [
        ...
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <div className="space-y-2">
              {gestorData.alertasCriticos.map((alerta, index) => (
                <div key={index} className="flex justify-between items-center p-2 bg-white rounded border-l-4 border-red-500">
                  <span className="text-gray-800">{alerta.message}</span>
                  <span className="text-sm text-gray-500">{alerta.time}</span>
                </div>
              ))}
            </div>
          </CardContent>
          ...
          <TabsContent value="performance" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-medium">Tempo Médio de Correção</span>
                    <span className="font-bold">2.3 dias</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-medium">ROI das Melhorias</span>
                    <span className="text-green-500 font-bold">+18%</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-medium">Lojas sem Visita (&gt; 7 dias)</span>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={[
                    { name: 'Visitas', meta: 30, realizado: 25 },
                    { name: 'Score Médio', meta: 90, realizado: 84 },
                    { name: 'Conformidade', meta: 95, realizado: 87 }
                  ]}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="meta" fill="#CBD5E0" name="Meta" />
